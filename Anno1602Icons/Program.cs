    ...
    public class Program
    {
        ...
        public static Bitmap RemoveBackground(Bitmap img, Bitmap median)
        {
            ...
            while (adj.Count > 0)
            {
                ...
                if (x < img.Width - 1 && ColorDifference(img.GetPixel(x + 1, y), median.GetPixel(x + 1, y)) < 0.1f && !adj.Contains(new Point(x + 1, y)))
                    adj.Enqueue(new Point(x + 1, y));
                if (y < img.Height - 1 && ColorDifference(img.GetPixel(x, y + 1), median.GetPixel(x, y + 1)) < 0.1f && !adj.Contains(new Point(x, y + 1)))
                    adj.Enqueue(new Point(x, y + 1));
                if (x > 0 && ColorDifference(img.GetPixel(x - 1, y), median.GetPixel(x - 1, y)) < 0.1f && !adj.Contains(new Point(x - 1, y)))
                    adj.Enqueue(new Point(x - 1, y));
                if (y > 0 && ColorDifference(img.GetPixel(x, y - 1), median.GetPixel(x, y - 1)) < 0.1f && !adj.Contains(new Point(x, y - 1)))
                    adj.Enqueue(new Point(x, y - 1));            
            }
            return res;
        }
